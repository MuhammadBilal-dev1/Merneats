import { MenuItem } from "@/types/restaurantType";
import { Button } from "./ui/button";
import { Card, CardContent, CardFooter } from "./ui/card";
import { userCartStore } from "@/store/useCartStore";
import { useNavigate } from "react-router-dom";

const AvaliablePages = ({ menus }: { menus: MenuItem[] }) => {
  const {addToCart} = userCartStore()
  const navigate = useNavigate()
  return (
    <div className="md:p-4">
      <h1 className="text-xl md:text-2xl font-extrabold mb-6">
        Available Menu
      </h1>
      <div className="grid md:grid-cols-3 gap-8">
        {menus?.map((menu: MenuItem) => (
          <Card key={menu._id} className="max-w-xs mx-auto shadow-lg rounded-lg overflow-hidden flex flex-col h-full">
            <img
              src={menu.image}
              alt=""
              className="w-full h-48 object-cover"
            />
            <CardContent className="p-4 flex-1 flex flex-col">
              <h2 className="text-xl font-semibold text-gray-800 dark:text-white line-clamp-1">
                {menu.name}
              </h2>
              <p className="text-sm text-gray-600 mt-2 line-clamp-3 flex-1">
                {menu.description}
              </p>
              <h3 className="text-lg font-semibold mt-4">
                Price: <span className="text-red-600">${menu.price}</span>
              </h3>
            </CardContent>
            <CardFooter className="p-4 mt-auto">
              <Button onClick={() => {
                addToCart(menu)
                navigate("/cart")
              }} className="w-full bg-orange hover:bg-hoverOrange">
                Add to cart
              </Button>
            </CardFooter>
          </Card>
        ))}
      </div>
    </div>
  );
};

export default AvaliablePages;
