...
const API_END_POINT = `${API_URL}/api/v1/restaurant`
...
export const useRestaurantStore = create<RestaurantState>()(persist((set, get) => ({
  ...
  searchRestaurant: async(searchText: string, searchQuery: string, selectedCuisines: any) => {
    try {
      ...
      if (response.data.success) {
        set({ loading: false, searchedRestaurants: response.data });
      }
    } catch (error) {
      set({loading: false})
    }
  },
  addMenuToRestaurant: (menus:MenuItem) => {
    set((state:any) => ({
      restaurant:  state.restaurant ? {...state.restaurant, menus:[...state.restaurant.menus, menus]} : null,
    }))
  },
  ...
  removeMenuFromRestaurant: (menuId: string) => {
    set((state: any) => {
      if (state.restaurant) {
        const updatedMenus = state.restaurant.menus.filter((menu: any) => menu._id !== menuId);
        return {
          restaurant: {
            ...state.restaurant,
            menus: updatedMenus
  ...
  setAppliedFilter: (value: string) => {
    set((state) => {
      ...
      const updateFilter = isAlreadyApplied ? state.appliedFilter.filter((item) => item !== value) : [...state.appliedFilter, value];
      return {
